...
## 4月12日 RAG 端到端生成质量评测（随机主题 Query）

### 实验设计

旧版评测（4月11日）使用从 chunk 反向生成的 query，天然偏向召回测试——query 与 chunk 一一对应，模型"作弊"概率高。本轮改为 **Gemini 生成的 50 条随机主题 query**（覆盖历史、科学、地理、文化等），不绑定特定 chunk，更接近真实用户场景。

评测维度（RAGAS 3 指标，Gemini 2.5 Flash 裁判）：
- **Faithfulness**：答案中的原子声明能否被 Context 支撑（反幻觉指标）
- **Answer Relevance**：答案与问题的语义相关度（反向生成 + BGE-M3 余弦相似度）
- **Context Precision**：召回的 Chunk 中有多少对回答有用（检索精准度）

对比组：
- RAG Top-1 / Top-3 / Top-5：分别送入 1/3/5 个 chunk 作为 Context
- Raw Answer：不提供任何 Context，纯模型裸答

生成模型：Qwen 9B（llama.cpp），刻意选小模型以放大 Context 的影响。

### 核心指标对比

| 指标 | RAG Top-1 | RAG Top-3 | RAG Top-5 | Raw (无RAG) |
|------|-----------|-----------|-----------|-------------|
| **Faithfulness** | 0.912 | **0.919** | 0.915 | 0.663 |
| Faith ≥0.67 达标率 | 92% | **94%** | 92% | 60% |
| **Answer Relevance** | 0.832 | **0.840** | 0.819 | 0.842 |
| **Ctx Precision** | **0.900** | 0.767 | 0.744 | 0.784 |
| Ctx Prec ≥0.67 达标率 | **90%** | 44% | 64% | 70% |
| **Overall** | **0.881** | 0.842 | 0.826 | 0.763 |
| 拒答率 | 40% | 24% | 12% | 0% |
| 原子声明 avg | 8.3 | 10.7 | 11.0 | 9.5 |
| prompt_tokens P50 | 367 | 848 | 1330 | 87 |
| gen_ms P50 | 1634 | 2095 | 2408 | 1601 |
| e2e_ms P50 | 4800 | 5226 | 5517 | 1601 |

### 分析

#### 1. RAG 确实有效：Faithfulness 从 0.663 → 0.91+

裸答 Faithfulness 只有 0.663，60% 达标率，且拒答率为 0%——模型在没有依据时也硬编答案，导致大量幻觉。RAG 三组的 Faithfulness 均 >0.91，达标率 >92%，说明 Context 有效约束了模型的输出，幻觉大幅减少。

#### 2. Top-1 Overall 最高（0.881），但拒答率偏高（40%）

Top-1 的 Context Precision 高达 0.900（reranker 排序质量好，排第一的 chunk 大概率是最相关的），但只给 1 个 chunk 导致信息量不足，模型频繁拒答。

#### 3. Top-5 拒答率最低（12%），但 Ctx Precision 被噪声拉低

多给 chunk 后模型有更多信息可用（拒答率 40%→12%），但额外 chunk 中混入无关内容，Ctx Precision 从 0.900 降至 0.744。

#### 4. Top-3 是综合平衡点

- Faithfulness 最高（0.919）
- 拒答率适中（24%）
- Ctx Precision 介于 Top-1 和 Top-5 之间（0.767）
- Answer Relevance 最高（0.840）

#### 5. Answer Relevance 四组差异不大（0.82~0.84）

说明问题理解能力主要取决于模型本身，Context 数量对此影响有限。

### 延迟影响

| 配置 | prompt_tokens P50 | gen_ms P50 | e2e_ms P50 |
|------|------------------|-----------|-----------|
| Top-1 | 367 | 1634 ms | 4800 ms |
| Top-3 | 848 | 2095 ms | 5226 ms |
| Top-5 | 1330 | 2408 ms | 5517 ms |

prompt 长度随 chunk 数线性增长，生成延迟也随之上升。Top-1 到 Top-5 的 e2e 差距约 700ms，相对检索层 3000ms 的耗时来说影响不大。

### 结论

- **RAG 有效**，Faithfulness 提升 ~37%（0.663→0.915）
- **推荐 Top-3** 作为默认配置：质量与拒答率的最佳平衡
- 后续优化方向：降低拒答率（优化 prompt 引导模型在信息不足时仍尽量回答）、提升 Ctx Precision（优化 reranker/MMR 策略过滤噪声 chunk）
...
## 4月9日 BM25 方案对比压测（tsvector vs pg_search）
