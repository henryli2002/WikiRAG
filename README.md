...
| 向量化耗时 | ~18 小时 42 分钟 |
| 向量化速度 | 21.02 条目/秒 |
| 最大显存占用 | ~7.66 GB (RTX 3070) |
| 输出 Parquet | 218 个文件 |
| 向量精度 | float16 (存储 & 推理) |
| 数据库导入耗时 | ~30 分钟 (MBP M4 Pro, Colima 6GB) |
...
| 方案 | 命令 | 特点 |
|------|------|------|
| HNSW (默认) | `python scripts/build_index.py` | 查询最快，内存占用最大 |
| HNSW + FP16 | `--quantize halfvec` | 内存减半，精度几乎无损 |
| HNSW + 二值量化 | `--quantize bit` | 极致压缩，精度有损 |
| HNSW 高精度 | `--m 32 --ef-construction 128` | 更高召回率，构建更慢 |
| IVFFlat | `--method ivfflat` | 构建快，查询稍慢 |
| IVFFlat 细分 | `--method ivfflat --lists 2000` | 更多聚类中心 |
| 降维 | `--dim 512 --quantize halfvec` | 截取前 N 维 + 半精度 |

索引名自动编码配置参数（如 `idx_emb_hnsw_halfvec_1024_m16_ef64`），不同配置可共存。检索 API 会自动检测当前索引类型，查询时匹配对应的精度（vector / halfvec / bit）。
