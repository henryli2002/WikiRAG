...
def get_next_file_index(output_dir):
    ...
    try:
        ...
        indices = [int(re.findall(r'\d+', f)[-1]) for f in existing_files if re.findall(r'\d+', f)]
...
def main():
    ...
    model = BGEM3FlagModel('D:/Project/CodeProject/embedding/models/bge-m3', use_fp16=True)
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
            ...
            try:
                ...
                for chunk_text in chunks:
                    ...
                    if len(batch_texts) >= batch_size:
                        # 显存监控下的向量计算
                        out = model.encode(batch_texts, batch_size=batch_size, max_length=1024)
                        ...
                        for i, meta in enumerate(batch_metas):
                            ...
                            if chunk_accumulator >= save_interval:
                                out_path = os.path.join(output_dir, f"wiki_embeddings_{file_index:04d}.parquet")
                ...
                pbar.update(1)
                if line_idx % 100 == 0:
                    pbar.set_postfix({'VRAM': f'{get_vram_usage():.2f}GB', 'FileIdx': file_index})
        ...
        # --- 4. 收尾工作：处理最后一批数据 ---
        if batch_texts:
            out = model.encode(batch_texts, batch_size=len(batch_texts), max_length=1024)
            vecs = out['dense_vecs'].astype(np.float16)
            for i, meta in enumerate(batch_metas):
                buffer_data['pageid'].append(meta[0])
                buffer_data['title'].append(meta[1])
